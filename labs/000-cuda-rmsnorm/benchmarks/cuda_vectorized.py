...
def run_benchmark(args: argparse.Namespace) -> None:
    ...
    print("== RMSNorm Scalar vs Float4 Vectorized ==")
    print(
        f"{'batch':>8} "
        f"{'hidden':>8} "
        f"{'scalar_min':>12} "
    ...
    for batch_size, hidden_size in shapes:
        ...
        scalar_out = rmsnorm_scalar(x, weight, args.eps)
        vector_out = rmsnorm_vectorized(x, weight, args.eps)

        torch.testing.assert_close(scalar_out, expected, rtol=1e-4, atol=1e-5)
        torch.testing.assert_close(vector_out, expected, rtol=1e-4, atol=1e-5)

        max_abs_diff = (vector_out - expected).abs().max().item()
        ...
        speedup_min = scalar["cuda_min_ms"] / vectorized["cuda_min_ms"]
        speedup_med = scalar["cuda_median_ms"] / vectorized["cuda_median_ms"]

        print(
            f"{batch_size:8d} "
            f"{hidden_size:8d} "
            f"{scalar['cuda_min_ms']:12.6f} "
            f"{scalar['cuda_median_ms']:12.6f} "
            f"{vectorized['cuda_min_ms']:12.6f} "
            f"{vectorized['cuda_median_ms']:12.6f} "
            f"{speedup_min:12.3f} "
            f"{speedup_med:12.3f} "
...
def run_profile(args: argparse.Namespace) -> None:
    ...
    fn = rmsnorm_vectorized if args.variant == "vectorized" else rmsnorm_scalar
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--variant", choices=["scalar", "vectorized"], default="vectorized")
