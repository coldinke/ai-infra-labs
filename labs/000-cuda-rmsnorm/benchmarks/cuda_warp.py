...
def run_benchmark(args: argparse.Namespace) -> None:
    ...
    print("== RMSNorm Scalar vs Warp Reduction ==")
    print(
        f"{'batch':>8} "
        f"{'hidden':>8} "
        f"{'scalar_min':>12} "
    ...
    for batch_size, hidden_size in shapes:
        ...
        warp_out = rmsnorm_warp(x, weight, args.eps)

        torch.testing.assert_close(scalar_out, expected, rtol=1e-4, atol=1e-5)
        torch.testing.assert_close(warp_out, expected, rtol=1e-4, atol=1e-5)

        max_abs_diff = (warp_out - expected).abs().max().item()
        ...
        speedup_min = scalar["cuda_min_ms"] / warp["cuda_min_ms"]
        speedup_med = scalar["cuda_median_ms"] / warp["cuda_median_ms"]
        ...
        print(
            ...
            f"{warp['cuda_min_ms']:12.6f} "
            f"{warp['cuda_median_ms']:12.6f} "
            f"{speedup_min:12.3f} "
            f"{speedup_med:12.3f} "
...
def run_profile(args: argparse.Namespace) -> None:
    ...
    fn = rmsnorm_warp if args.variant == "warp" else rmsnorm_scalar
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--variant", choices=["scalar", "warp"], default="warp")
