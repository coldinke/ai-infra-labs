...
torch::Tensor rmsnorm_cuda_vectorized_warp(torch::Tensor x, torch::Tensor weight, double eps);
...
torch::Tensor rmsnorm_forward_vectorized_warp(torch::Tensor x, torch::Tensor weight, double eps) {
  CHECK_INPUT(x);
  CHECK_INPUT(weight);
  ...
  return rmsnorm_cuda_vectorized_warp(x, weight, eps);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def(
    "forward_vectorized_warp",
    &rmsnorm_forward_vectorized_warp,
    "Float4 vectorized + warp-level reduction RMSNorm forward (CUDA, fp32)");
